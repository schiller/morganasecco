PostUpdateController = RouteController.extend({
	...
	waitOn: function () {
		Meteor.subscribe('postsAdmin', {slug: this.params.slug});
	},
	data: function () {
...
PostShowController = RouteController.extend({
	layoutTemplate: 'appLayout',
	waitOn: function () {
		Meteor.subscribe('posts', {slug: this.params.slug});
	},
	data: function () {
...
PostAdminController = RouteController.extend({
	layoutTemplate: 'appLayout',
	waitOn: function () {
		Meteor.subscribe('postsAdmin');
	},
	data: function () {
		return {posts: Posts.find({}, {sort: {createdAt: -1, updatedAt: -1}})};
	},
	action: function () {
